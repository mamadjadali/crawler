...
import { updateSettingsFee, updateSettingsProfit } from '@/actions/feeUpdate'
...
interface EditableProfitProps {
  settings: Setting
}

export default function EditableProfit({ settings }: EditableProfitProps) {
  const router = useRouter()
  const [editing, setEditing] = useState(false)
  const [value, setValue] = useState(settings.profit ?? 0)
  const [displayValue, setDisplayValue] = useState(settings.profit ?? 0)
  const [loading, setLoading] = useState(false)

  const handleSave = async () => {
    setLoading(true)
    try {
      await updateSettingsProfit({ profit: value })
  ...
  return (
    <div className="flex items-center justify-between border-none bg-white rounded-[10px] p-2">
      <div className="text-base flex items-center gap-2 font-medium text-[#212a72]">
        <TbTrendingUp3 className="size-5 text-green-700" />
        ســود
      </div>
