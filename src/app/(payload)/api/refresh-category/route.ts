...
export async function POST(req: Request) {
  ...
  try {
    ...
    const products = docs as ProductLink[]
      ...
      products.map((product) =>
        productLimiter(async () => {
          ...
          if (now - lastUpdate < MIN_REFRESH_INTERVAL_MS || !product.productUrls?.length) {
            skipped++
            log(
              `[SKIP - Recent] ${prodId} - ${prodName} (updated ${Math.round(
          ...
          try {
            const updatedUrls: ProductLink['productUrls'] = await Promise.all(
              product.productUrls.map((urlEntry) =>
                urlLimiter(async () => {
                  if (!urlEntry.url) return urlEntry as ProductLink['productUrls'][number]
                  ...
                  try {
                    ...
                    return {
                      ...
                      crawlError: result.success ? null : (result.error ?? 'Error'),
                      priceHistory: newHistory,
                    } as ProductLink['productUrls'][number]
                  } catch (err) {
                    return {
                      ...urlEntry,
                      crawlStatus: 'failed',
                      crawlError: err instanceof Error ? err.message : 'Error',
                    } as ProductLink['productUrls'][number]
                  }
                }),
              ),
            ...
            failed++
            log(`[FAILED] ${prodId} - ${prodName}: ${err instanceof Error ? err.message : 'Error'}`)
          }
        }),
      ),
    )
...
//     const { docs: products } = await payload.find<ProductLink>({
//       collection: 'product-links',
//       where: {
//         disable: { not_equals: true },
//         category: { equals: category },
